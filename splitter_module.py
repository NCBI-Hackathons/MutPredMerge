...
def processing_exonic_variant_function(path, suffix, threads):
    ...
    for i in range(threads):
        split_indels[i].to_csv("intermediates/splits/%s.indels_%s.exonic_variant_function" % (suffix, i), sep="\t", header=False, index=False)
    ...
    for i in range(threads):
        split_LOF[i].to_csv("intermediates/splits/%s.LOF_%s.exonic_variant_function" % (suffix, i), sep="\t", header=False, index=False)
